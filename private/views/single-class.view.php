...
<?php $this->view('includes/nav')?>
	...
	<div class="container-fluid p-4 shadow mx-auto" style="max-width: 1000px;">
		...
		<div class="row">
	 	<center><h4><?=esc(ucwords($row->class))?></h4></center>
			<table class="table table-hover table-striped table-bordered">
				<tr><th>Creado por:</th><td><?=esc($row->user->firstname)?> <?=esc($row->user->lastname)?></td>
				<th>Fecha creacion:</th><td><?=get_date($row->date)?></td></tr>
			...
			<ul class="nav nav-tabs">
				<?php if(Auth::access('lecturer')):?>
				  ...
				  <li class="nav-item">
				    <a class="nav-link <?=$page_tab=='lecturers'?'active':'';?> " href="<?=ROOT?>/single_class/<?=$row->class_id?>?tab=lecturers">Profesores</a>
				  </li>
				<?php endif;?>

			  <li class="nav-item">
			    <a class="nav-link <?=$page_tab=='students'?'active':'';?> " href="<?=ROOT?>/single_class/<?=$row->class_id?>?tab=students">Estudiantes</a>
			  </li>

			  <?php if(Auth::access('lecturer')):?>
				  <li class="nav-item">
				    <a class="nav-link <?=$page_tab=='tests'?'active':'';?> " href="<?=ROOT?>/single_class/<?=$row->class_id?>?tab=tests">Pruebas</a>
				  </li>
		 		<?php endif;?>
		...
		<?php else:?>
			<center><h4>El curso no ha sido encontrado</h4></center>
		<?php endif;?>
